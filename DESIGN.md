...
components:
  ...
  panel-mode-build-custom:
    ...
    boxShadow: "0 4px 16px rgba(0,0,0,0.10) — a soft drop shadow, NOT an inset hairline edge. This is the shared mode-panel overlay treatment (see panel-mode-compounds and Elevation & Depth): both mode panels float over the calculator card rather than nesting into it, and the drop shadow — the one drop shadow in the system — is what marks that. No border, no inset ring alongside it."
    zIndex: 10
  ...
  panel-mode-compounds:
    ...
    boxShadow: "0 4px 16px rgba(0,0,0,0.10) at rest — the same drop shadow as panel-mode-build-custom, the shared mode-panel overlay treatment. NO resting inset ring or border. The 2px {colors.primary} focus-visible ring is KEPT (`focus-visible:inset-ring-2 focus-visible:inset-ring-primary`): unlike the Build custom panel, THIS panel element is itself the focusable listbox (`tabindex=0 role=listbox`), so it is the target keyboard focus lands on and needs a visible focus cue of its own — the option highlight does not serve that role (it is shown whenever the panel is open, not only when focused). Do not drop the focus-visible ring."
    zIndex: 10
...
**Key Characteristics:**
- Light warm canvas with a four-step surface ladder for hierarchy — no drop shadows anywhere, save one deliberate exception: the two Molar Mass mode panels, which are `position: fixed` overlays and use a soft drop shadow to mark that they float over the card (see Elevation & Depth)
...
- **Surface 1** (`{colors.surface-1}` #ffffff): One step above canvas — the calculator card, input field interiors, and both Molar Mass mode panels (`position: fixed` overlays — they carry a drop shadow instead of an inset edge, see Elevation & Depth).
...
- **Shared overlay treatment.** Both panels are `{colors.surface-1}` #ffffff with the drop shadow `0 4px 16px rgba(0,0,0,0.10)` in place of a resting edge — no border, no resting inset ring; the one drop shadow in the system (see Elevation & Depth). The only divergence is the focus-visible ring, Compounds-only, covered in its bullet below.
- **Build custom** uses the `panel-mode-build-custom` recipe (`{rounded.lg}`, 16/20px padding, plus the shared overlay fill+shadow) and holds a search input, the element grid, a Formula / Molar Mass readout, and a **"Use this molar mass" button** (`button-primary`). The element tiles inside it are `{colors.canvas}` #FAF9F5 — a step *below* the panel — so the white-tile-on-white-panel flatness is avoided by making the tile the recess, not the lift (see Elevation & Depth). There is still no confirm *step* — every tile click writes the slot, which the field is already a live view of — so the button's only job is closing the panel deliberately; see **Dismissal** for why it's the *only* thing that closes this panel.
- **Compounds** has no container of its own: **the panel *is* the listbox** (`panel-mode-compounds`: the shared overlay fill+shadow, `{rounded.md}`, `max-h-72 overflow-y-auto`). Because the panel element itself is the focusable listbox (`tabindex="0" role="listbox"`), it additionally carries `focus-visible:inset-ring-2 focus-visible:inset-ring-primary` — the keyboard-focus cue it needs and the Build custom panel doesn't (that panel is never itself focused). See the deviation note below. Selecting does **not** close it — see **Dismissal**.
...
- **No border, no fill of its own** — the header floats directly on `{colors.canvas}` with no dividing line separating it from the page below. It previously carried a `border-b border-hairline-soft`; that line is deliberately removed, not an oversight. Separation from the content below now comes entirely from the calculator card's own surface (`{colors.surface-1}` white against canvas) rather than a header rule — don't reintroduce a border here to "fix" perceived flatness. Logo-bottom to card-top: **44px at `mobile:` (≥520px) and up, 36px below it** — a 1px reduction from when the border existed (45px/37px), matching the removed line's own width exactly; nothing else in the spacing changed.
...
- The explainer/theory section below the calculator has no component definitions yet — that section's design is deliberately deferred. A few conventions are decided, though:
  - Sub-items within a section (the explainer's four blocks, the FAQ's five Q&A pairs) are separated by spacing alone — `py-4` (`{spacing.md}`, 16px per side) per item, no `border-t` hairline between them. Follow this for any future addition to either section rather than reintroducing a divider line.
  - Box-style content within these two sections — the worked-example result boxes (explainer's live example, both FAQ grams↔moles examples) and the formula-triangle card — fill with `{colors.surface-3}` (#EFEDE7) and carry no border/inset-ring. Fill-only is the convention for this kind of box here; don't add a ring back in.
  - Section titles ("Understanding Moles", "FAQs", both `<h2>`) are `1.4rem` / weight 800. Sub-item titles (the explainer's four block headings, the FAQ's five question headings, all `<h3>`) are `1.3rem` / weight 600. Neither size sits on the standard Tailwind scale, so both are set via arbitrary value syntax (`text-[1.4rem]` / `text-[1.3rem]`) rather than approximated to the nearest step — match that when adding more titles here.
- `{colors.surface-3}` and `{colors.surface-4}` both have consumers and are no longer listed as unused headroom: `surface-3` fills `molar-mass-mode-trigger` (the left half of the fused Molar Mass control, one step up from the input beside it), the element tile's symbol chip (one step below the #FAF9F5 tile — a shallow inset), and — outside the calculator — the explainer/FAQ worked-example boxes and the formula-triangle card (see above); `surface-4` fills only the element tile's stepper hover circle now (the symbol chip stepped off it when the Build custom panel was re-grounded — panel #ffffff, tile `{colors.canvas}`, chip `{colors.surface-3}`).
