...
- **Build custom** uses the `panel-mode-build-custom` recipe (`{colors.surface-2}`, `{rounded.lg}`, 16/20px padding, hairline inset ring) and holds a close button, a search input, the element grid, and a Formula / Molar Mass readout. **There is no confirm button** — every tile click writes the slot, which the field is a live view of, so a "Use this molar mass" step would have nothing left to do. With that button gone the two readouts fit side by side at every width, so the readout row does not stack below 520px.
- **Compounds** has no container of its own: **the panel *is* the listbox**, so it carries the popup recipe directly (`panel-mode-compounds`: `{colors.surface-1}` a step above the card it floats over, hairline ring, `max-h-72 overflow-y-auto`). Selecting does **not** close it — a selection and a dismissal are different intents, and closing on every pick would turn comparing two compounds into a two-step "reopen, then pick" action each time. Closing is *available*, just not automatic on selection — see **Dismissal**.

**Dismissal.** Both panels reversed course from their original stage 3/4 framing ("the panel is the mode's persistent UI, not a dismissible popup") after usability testing showed people expected a way out that wasn't "go find the dropdown again." Three paths, converging on one function (`closeMolarMassModePanel()`), so all three have exactly the same effect — switch `activeMolarMassMode` back to `'type-in'`, which hides the panel, updates the trigger label, and flips `#molar-mass` back to editable, precisely as if "Type in" had been picked from the dropdown:

- **Escape**, from anywhere inside the panel (the search input included) or the mode trigger. With the mode dropdown *also* open over a panel, Escape closes the dropdown first and a second Escape reaches the panel — same "innermost popup first" rule the dropdown already used for its own Enter/Home/End/Arrow* keys.
- **Clicking outside** the panel — reusing the same capture-phase document-click pattern the mode dropdown's own listbox uses (and the retired accordion's `preset-listbox` used before it), scoped by DOM containment so a click on the panel itself, the mode wrapper, or the mode listbox is never mistaken for "outside." Deliberately does not refocus the trigger afterward: the user's click landed somewhere real, and forcing focus back would fight it.
- **A close (X) button** on each panel — `p-2.5 mobile:p-0` around a 24px `rounded-full` circle, the same hit-area-separate-from-painted-box shape as every other icon button here (element-tile-stepper, the Molar Mass/Mass/Moles steppers), with the same `group-hover:bg-hairline-tertiary` fill. Build custom's sits in its own header row above the search input. Compounds has no room for a header without disturbing the option list, so its button is a `position: sticky` first child *inside* the listbox rather than a wrapper around it — deliberately not a `role="option"` (so it's inert to the component's own option-index math) and reachable by its own click handler or by Tab. Both close buttons refocus the trigger afterward, the same reasoning as Escape: the control that had focus just disappeared, and the trigger is the sensible place for it to land.

Escape and the close button refocus the trigger; click-outside does not. That split is deliberate, not an oversight — see the bullet above for why.
...
- **The mode panels' close (X) buttons** follow the quantity-stepper pattern above, not `button-copy`'s: `p-2.5 mobile:p-0` around a 24×24px `rounded-full` inner span, so touch gets a 44×44px target and desktop gets a tight 24×24px one, keyed to the same `mobile` breakpoint. `button-copy` was the other candidate (a flat 44×44px at every width) but was set aside — a dismiss action invites a hover/press affordance the way the steppers do, where `button-copy` deliberately has none (see its own entry above).
