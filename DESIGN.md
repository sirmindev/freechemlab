...
- **Option states:** highlighted (keyboard-active or hovered) gets `bg-surface-2`, same fill as `pill-inactive-hover`. A committed selection (`aria-selected="true"`) would get `bg-primary-soft` / `text-primary-pressed` — the exact reserved future use for `{colors.primary-soft}` noted under Colors — but in practice this state is never visible to the user: see the selection-behavior note below.
- **Selection behavior — no confirm button on this tab.** Clicking an option, or highlighting it and pressing Enter/Space, immediately writes `preset.molarMass` into the Molar Mass input (dispatching a real `input` event so the calculation re-runs) and then collapses the whole Browse Elements panel — the same end effect the old select-then-"Use this molar mass" flow had, just triggered by the selection itself rather than a second click. The picker's selection state is reset to the placeholder immediately after, so it always reopens fresh — this mirrors the old `<select>` being reset to `value = ''` in the same spot. The "Use this molar mass" button still exists on the sibling Build-custom tab (`use-custom`) for that tab's multi-element flow; it is unrelated to this control and was not touched.
