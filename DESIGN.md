...
- **Build custom** uses the `panel-mode-build-custom` recipe (`{colors.surface-2}`, `{rounded.lg}`, 16/20px padding, hairline inset ring) and holds a search input, the element grid, a Formula / Molar Mass readout, and a **"Use this molar mass" button** (`button-primary`). There is still no confirm *step* — every tile click writes the slot, which the field is already a live view of — so the button's only job is closing the panel deliberately; see **Dismissal** for why it's the *only* thing that closes this panel.
