...
- The element grid inside Browse Elements is **not** a fixed column count, and it is **not** `1fr`. Column count and width are computed in JS against the measured container width (`layoutElementGrid()`, re-run by a `ResizeObserver`), so both always resolve to **whole pixels**:
  1. `n = floor((W + minGap) / (target + minGap))`, target **252px**, minGap 8px.
  2. Reduce `n` while a column would fall below the 222px truncation floor.
  3. `colW = floor((W − (n−1)·minGap) / n)`, then walk `colW` down until the leftover divides evenly across the `n−1` gaps. **The slack lands in the gaps; the columns stay uniform integers.**
- At desktop the container is **772px**, giving **3 × 252px with an 8px gap** — 756 + 16 = 772 exactly, no remainder, and 772 ÷ 4 = 193 so the container is 4pt-aligned. Verified identical in headed Chromium, headless Chromium **and** Firefox.
- **The scroll wrapper's native scrollbar is hidden** (`scrollbar-width: none` + `::-webkit-scrollbar { display: none }`) so it consumes **zero** layout width, and the wrapper carries **no right padding**. That is what makes 772 reachable and browser-independent — see "Why the scrollbar is hidden" below.

#### Why the scrollbar is hidden

This was a live layout bug, not a theoretical one. The wrapper is `overflow-y: auto`, and the native scrollbar's width is **not controllable across browsers**:

- Chrome and Edge reserve a **classic 15px** scrollbar. That dropped the wrapper from 772 → 757, minus the old `pr-0.5` → **755**, and `floor((755 + 8) / 258) = 2`. **Desktop Chrome and Edge were rendering the grid at 2 columns instead of 3.** Firefox kept 3 because its scrollbar took no width. Reproduced here in headed Chromium: container 755, template `373px 373px`.
- **Headless Chromium uses overlay scrollbars and reports 0px, hiding the bug completely.** Any measurement of this container taken headless is not evidence about Chrome or Edge. Always cross-check headed.
...
**Zero is the only scrollbar width that is identical in every browser**, so the scrollbar is hidden rather than sized. Scrolling itself is unaffected and was verified in headed Chromium: wheel (0 → 240), Tab (focused tile scrolled into view, `scrollTop` 226), PageDown (226 → 506) and ArrowDown (506 → 546) all work.

> There is deliberately **no replacement scroll affordance** yet. Hiding the scrollbar removes the only visual cue that the list scrolls; choosing a replacement is an open design decision, not an oversight.

#### What 772 depends on

The deduction chain from `main` is: `896 − 32 (main px-4) − 2 (card border) − 48 (card p-6) − 2 (panel border) − 40 (panel-custom p-5) − 0 (scrollbar) − 0 (wrapper padding) = 772`.
...
- **The 375px viewport cannot be 4pt-aligned**, because 375 is itself odd and `main` is viewport-driven below the `max-w-4xl` cap. Measured: 375 − 32 = 343 (odd), and the odd parity propagates the whole way down to a 251px container (251 % 4 = 3). This is inherent to the viewport width and is not corrected.
...
- The 222px floor is set by the widest *selected* tile, not by taste. Measured empirically rather than derived: with "Molybdenum" (the longest name in the 63-element dataset) selected, the name begins to truncate once the column falls **to 222px or below**. The computed layout delivers 252px, leaving **30px of headroom**.
- **`GRID_MIN_COL` is currently non-binding and should stay anyway.** Because `n = floor((W + 8) / 260)` implies `W ≥ 260n − 8`, the resulting column is `colW ≥ floor(252n / n) = 252`, which can never fall below 222 — so neither the `while` guard nor the loop's lower bound can fire at the current target. It is kept as a documented safety net: it is the real measured truncation threshold, and it becomes load-bearing again the moment the target is lowered below 222. Do not delete it because it looks unused, and do not raise it to match the target — those are two different numbers measuring two different things. The unselected tile needs far less, but sizing to that would make every tile truncate the moment it is selected, so the floor is set by the selected case.
...
- **4-up does not fit and cannot be made to fit by shrinking type.** At 4-up the panel yields 187px columns, of which the fixed furniture (chip, stepper, padding, gaps) consumes 152px, leaving **35px** for the name against the 85px it needs. Fitting "Molybdenum" into 35px requires roughly a 5.8px font — less than half the system's 12px floor. Even deleting the stepper's border and shrinking it to 60px only buys a 9.3px font. 4-up needs a grid about **924px** wide; this panel is **772px**. The 4-column mockup is drawn on a wider canvas than the app's actual panel and only ever shows "Hydrogen", which is 62px — comfortably inside a width that "Molybdenum" overruns by 23px.
...
- **Glyph centring is a layout property, not an icon property.** The per-column inconsistency that once made the **+** look off-centre came from fractional `1fr` columns, and is fixed by the integer-column algorithm under Grid & Container — not by touching the icons. Measured at 10× across the first, middle and last column of a row, the plus ink offset is now `[−0.35, −0.35]` in all three, identical at 1280 / 900 / 640 / 390px viewports.
- The remaining constant −0.35px is shared by **every** tile and **both** glyphs, so it produces no visible inconsistency. It originates inside the icon itself — the SVG box is 13.2969px inside a 20px wrapper — and is within the noise of threshold-based ink detection. Making the SVG an integer size would remove it, at the cost of changing the glyph size. Do not "fix" it by nudging one glyph: the measurements show no asymmetry between + and −, so a nudge would create one.
...
- Quantity steppers (− / +) in Build custom paint a 24×24px box at every viewport. The hit area and the painted box are deliberately separate: the `<button>` is a transparent wrapper around a 24×24 inner span that carries the radius and the hover fill. On desktop the wrapper adds no padding (24×24 total); on touch it adds **10px** on all sides, giving a 44×44px target while the painted box stays 24×24. Measured at a 390px viewport: hit area 44×44, painted box 24×24. The control looks identical at both widths — only the wrapper's padding changes, which is why the fill must live on the inner span and never on the button itself. Shrinking the visual size is always paid for by growing the wrapper's padding, never by letting the target shrink with it.
