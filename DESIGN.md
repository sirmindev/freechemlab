...
- **They reposition on scroll/resize, they do not dismiss.** A panel stays open for as long as its mode is selected, so it has to track the field moving under it. Capture-phase listeners, attached while a panel is active and removed when the mode returns to Type in.
...
- **Build custom** uses the `panel-mode-build-custom` recipe (`{colors.surface-2}`, `{rounded.lg}`, 16/20px padding, hairline inset ring) and holds a search input, the element grid, and a Formula / Molar Mass readout. **There is no confirm button** — every tile click writes the slot, which the field is a live view of, so a "Use this molar mass" step would have nothing left to do. With the button gone the two readouts fit side by side at every width, so the readout row does not stack below 520px.
- **Compounds** has no container of its own: **the panel *is* the listbox**, so it carries the popup recipe directly (`panel-mode-compounds`: `{colors.surface-1}` a step above the card it floats over, hairline ring, `max-h-72 overflow-y-auto`). Selecting does **not** close it — it is the mode's UI, not a popup, and closing would leave no way back without switching mode twice.
...
- **Keyboard (as the Compounds panel drives it):** ArrowDown/Up move the highlight; Home/End jump to first/last; Enter/Space commits the highlighted option. There is no open/close, so Escape and Tab have nothing to collapse and are left to the browser. Typeahead: any single printable keypress buffers into a 500ms-debounced string and jumps the highlight to the next option (wrapping) whose name starts with it, searching from the current highlight.
