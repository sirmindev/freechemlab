...
- The element grid inside Browse Elements is **not** a fixed column count, and it is **not** `1fr`. Column count and width are computed in JS against the measured container width (`layoutElementGrid()`, re-run by a `ResizeObserver`), so both always resolve to **whole pixels**:
  1. `n = floor((W + minGap) / (target + minGap))`, target 250px, minGap 8px.
...
- At the 770px desktop panel this gives **3 × 250px with a 10px gap** — 750 + 20 = 770 exactly, no remainder. Measured: 253px × 2 (gap 8) at a 640px viewport, 264px × 1 at 390px. Every tile starts on a whole pixel at every width.
...
- The 222px floor is set by the widest *selected* tile, not by taste. Measured empirically rather than derived: with "Molybdenum" (the longest name in the 63-element dataset) selected, the name begins to truncate once the column falls **to 222px or below**. The computed layout delivers 250px, leaving **28px of headroom**. The unselected tile needs far less, but sizing to that would make every tile truncate the moment it is selected, so the floor is set by the selected case.
