...
- **Element tile names still truncate on narrow phones, and this is unresolved.** Measured in headed Chromium: "Molybdenum" truncates at **375–415px** (clean from 416), "Hydrogen" at **375–395px** (clean from 396). At 375 the tile is 221px wide and the 44px touch wrapper makes the stepper 108px, leaving the body 87px — 32px chip + 12px gap = 44, so 43px for a name needing 64–85px. Nothing truncates at 425 or above at any viewport up to 1920. **Fixing the `md` → `mobile` breakpoint did not change this band**, because the whole band sits below 520 where the touch padding applied both before and after; the fix only affects 520–767. The `GRID_MIN_COL = 222` floor under Grid & Container was derived from the 24px desktop stepper and does not cover the 108px touch case. Resolving this is a design decision — shrink the touch target, drop the mass line, allow two lines, or accept the ellipsis — and has not been made.
