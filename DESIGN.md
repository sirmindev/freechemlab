...
- **Surface 2** (`{colors.surface-2}` #F5F3EE): Two steps — the input and result field containers, and the Build custom mode panel.
...
| 2 (nested lift) | `{colors.surface-2}`, `box-shadow: inset 0 0 0 0.5px {colors.hairline}` | Field containers, Build custom mode panel |
...
**`button-primary`** — Green pill. Currently unused: its last consumer was the removed "Use this molar mass" confirm button, and no mode panel has a confirm step. Kept as the defined recipe for the next primary action rather than deleted, since the token and the spec are still the answer to "what does a primary button look like here".
...
- **They reposition on scroll/resize, and they dismiss.** A panel stays open for as long as its mode is selected — repositioning on scroll while active is unrelated to that — but it is no longer true that the dropdown is the only way to leave one. Scroll/resize listeners are capture-phase, attached while a panel is active and removed when the mode returns to Type in; see **Dismissal** below for how "returns to Type in" now happens.
- **Returning to a mode repaints from the slot, not from what the panel was left holding.** Neither the grid nor the picker is torn down on hide, so both keep their own internal state across a hide/show — reading from that instead of from the store looks identical in the common case and silently diverges the moment anything else writes the slot. Both restore paths read the store; both are covered by a test that overwrites the slot *while the mode is inactive*, which is what separates "restored from the store" from "never lost it". This is exactly what makes closing a panel non-destructive: closing only re-points which slot `#molar-mass` is a view of (see Per-mode state above) — it does not clear the slot the user was just looking at, so reopening the mode shows exactly what they left.
- **Build custom** uses the `panel-mode-build-custom` recipe (`{colors.surface-2}`, `{rounded.lg}`, 16/20px padding, hairline inset ring) and holds a search input, the element grid, and a Formula / Molar Mass readout. **There is no confirm button** — every tile click writes the slot, which the field is a live view of, so a "Use this molar mass" step would have nothing left to do. It also never auto-closes on a tile pick — see **Dismissal** for why that's a deliberate difference from Compounds, not an inconsistency.
- **Compounds** has no container of its own: **the panel *is* the listbox**, so it carries the popup recipe directly (`panel-mode-compounds`: `{colors.surface-1}` a step above the card it floats over, hairline ring, `max-h-72 overflow-y-auto`). Selecting **does** close it, immediately — see **Dismissal**.

**Dismissal.** Both panels reversed course from their original stage 3/4 framing ("the panel is the mode's persistent UI, not a dismissible popup") after usability testing showed people expected a way out that wasn't "go find the dropdown again." Four paths exist, not three of a kind: three are a genuine *full close* (switch `activeMolarMassMode` back to `'type-in'`, which hides the panel, updates the trigger label, and flips `#molar-mass` back to editable, precisely as if "Type in" had been picked from the dropdown), and the fourth — Compounds' auto-close — deliberately is not, for a reason worth reading before touching any of this:

- **Escape**, from anywhere inside the panel (the search input included) or the mode trigger. With the mode dropdown *also* open over a panel, Escape closes the dropdown first and a second Escape reaches the panel — same "innermost popup first" rule the dropdown already used for its own Enter/Home/End/Arrow* keys. A full close: converges on `closeMolarMassModePanel()`.
- **Clicking outside** the panel — reusing the same capture-phase document-click pattern the mode dropdown's own listbox uses (and the retired accordion's `preset-listbox` used before it), scoped by DOM containment so a click on the panel itself, the mode wrapper, or the mode listbox is never mistaken for "outside." Deliberately does not refocus the trigger afterward: the user's click landed somewhere real, and forcing focus back would fight it. A full close.
- **Toggle-close**: re-picking the *already-active* mode's own option from the dropdown. Picking a mode that isn't already active is a normal switch; picking the one that is would otherwise be a pointless no-op repaint, so `selectMolarMassMode()` reads it as "close instead" — the same full close as Escape/click-outside, gated behind `isModePanelOpen()` so it only fires while there's actually something open (see the next bullet for why that check exists).
- **Auto-close on pick, Compounds only.** Selecting a compound hides the panel in the same tick it fills `#molar-mass` — but does **not** call `closeMolarMassModePanel()` and does **not** switch `activeMolarMassMode` to `'type-in'`. If it did, the field would flip to Type in's own (unrelated, often empty) slot the instant after showing the picked value, undoing the fill it just did. Compounds instead gets its own `compoundsPanelOpen` flag: the mode stays `'compounds'`, the trigger keeps reading "Compounds", `#molar-mass` keeps showing the compounds slot — only the panel collapses. `isModePanelOpen(mode)` is the one function every visibility decision (show/hide, which panel scroll/outside-click listeners protect, and toggle-close's own close-vs-reopen branch) reads, so those three can't disagree about what "open" means for a mode that can be active-but-collapsed. **Re-picking "Compounds" from the dropdown while collapsed reopens it** rather than toggle-closing further — toggle-close's `isModePanelOpen()` gate is what tells the two apart. Build custom has no equivalent flag: a tile pick is one of several in progress, not a complete answer the way a compound pick is, so its panel only ever leaves via the three full-close paths above.

Escape and toggle-close refocus the trigger (the control that had focus just disappeared); click-outside does not (the user's click landed somewhere real); auto-close does neither (mode didn't change, so there's nothing displaced to refocus).
...
- **Keyboard (as the Compounds panel drives it):** ArrowDown/Up move the highlight; Home/End jump to first/last; Enter/Space commits the highlighted option. Tab has nothing to collapse and is left to the browser. Escape does now dismiss the panel, but that binding lives on the host, not the component — see Dismissal under Cards & Panels — since closing is a lifecycle decision and this component doesn't own the lifecycle (see "Where the split is" above). Typeahead: any single printable keypress buffers into a 500ms-debounced string and jumps the highlight to the next option (wrapping) whose name starts with it, searching from the current highlight.
