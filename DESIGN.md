...
- Browse Elements panel tab content padding (Build custom and Presets): 16px below 520px, 20px (not a spacing token) at `mobile:` (≥520px) and up. Matches the outer card's mobile-reduction pattern; the panel's own desktop value stays a bespoke 20px, not 24px.
...
**`segmented-tab`** + **`segmented-tab-active`** — Build custom / Presets inside the Browse Elements panel.
...
- **Tab order (Build custom left, Presets right) is independent of which tab is active by default.** Build custom is the tab shown on panel open — that's driven by which classes/`hidden` state are hardcoded onto `#tab-custom`/`#panel-custom` specifically (by ID) and by the JS `activeTab` default (`'custom'`), none of which derive from DOM position. Each tab's underline is its own static `border-bottom`, toggled by a full className swap on that specific button — not a shared indicator computed from index/position — so either the order or the default can change independently without touching the other or risking any position-dependent logic. There is no `nth-child`/index-based selector anywhere in this component. (Presets was the default before Build custom; changing which tab defaults active means keeping several places in sync — `activeTab`'s initial value, which of the two tab buttons carries the active classes in the static markup, and which of the two panels carries `hidden` in the static markup — check all of them are still consistent if this is ever revisited.)
...
- **Opening the panel scrolls it into view**, deferred one `requestAnimationFrame` after unhiding so the panel — and, on the Custom tab, the just-rendered element grid — has committed layout before the browser measures where to scroll. Without this the newly-revealed content could extend below the fold with no indication there's more to see. Fires **only** on the trigger's open action — never on close (no scroll call in that branch; any scroll-position change on close is the browser passively re-clamping to a shorter document once the panel's content is hidden, not an active scroll), never on switching between Presets/Build custom, and never on search filtering within the Custom tab. There is no sticky/fixed header on this page to offset against; if one is ever added, this call needs a compensating `scroll-margin-top` or offset.
  - **Desktop (≥520px) uses `browsePanel.scrollIntoView({ behavior: 'smooth', block: 'start' })` unchanged** — its panel is comfortably shorter than its viewport (measured 384px distance at 768/1024/1440×900), so top-aligning it doesn't cost anything worth optimizing.
  - **Below 520px the target is computed manually instead of via `scrollIntoView`'s own `'nearest'`**, because native `'nearest'` doesn't know about content that isn't in the DOM's measured height yet. The math: `startTarget` (full `'start'`-equivalent, `scrollY + panelRect.top`) and `nearestTarget` (`scrollY + panelRect.bottom - innerHeight`, i.e. what `'nearest'` would pick when it chooses bottom-alignment, which it does here — the panel is shorter than the viewport, so bottom-align always needs less scroll than top-align). Final target is `min(startTarget, nearestTarget + reserve)` — see below for `reserve`. The cap at `startTarget` means this **never scrolls further than plain `'start'` would**, which matters because `'start'`'s always-maximal jump (335px at 375×667) was the exact complaint that motivated moving off it in the first place.
  - **`reserve` accounts for the Build-custom tab's "Use this molar mass" button, which is still `hidden` (zero height) at scroll time** — nothing is selected yet, so the button isn't part of the panel height either `'nearest'` or the manual target math would otherwise measure against. Left unaccounted, `'nearest'`'s bottom-alignment zeroes out all slack below the panel (measured: panel bottom landed flush with the viewport, 0px to spare), so the button — which appears later, on selection, with no scroll of its own to reveal it — rendered partly or fully below the fold (measured 40px of overflow at every mobile width/height tested). `reserve` is **measured live**, not hardcoded: toggle `useCustomBtn`'s `hidden` class off, diff `#browse-panel`'s height before/after, toggle back — all synchronous within the same `requestAnimationFrame` callback, before the frame paints, so there's no visible flash. This measured **56px consistently across 320/375/420/519px** (the button's own 44px height plus a fixed margin above it — not data-dependent, so a live measurement isn't overkill here, it's just avoiding a magic number that would go stale the next time that button's size changes). If the button is already visible when the panel opens (reopening after a prior selection persisted), `reserve` is naturally `0` — the panel's measured height already includes it. **This only applies to `activeTab === 'custom'`** — the Presets tab (`preset-listbox`, see below) has no equivalent confirm button to defer for: selecting a compound there fills Molar Mass and collapses the whole panel in the same synchronous call, so there's nothing that can still be `hidden` at scroll time and appear later. `btn` resolves to `null` on that tab and `reserve` stays `0`.
  - **Net effect at 320/375/420×667–844: 315px scroll distance** (up from the reserve-less 259px, still well under `'start'`'s 335px), with the tab bar, search input, and — after selecting an element — the "Use this molar mass" button all fully on screen (verified, not assumed).
  - **Trade-off, stated plainly: on a viewport too short for panel + reserve to both fit (measured 375×568, panel content alone is 647px), the button can still land partly below the fold after a selection** (measured 63px of overflow there) — the `min(...)` cap intentionally refuses to scroll past `'start'` to chase it, because doing so would mean scrolling the tabs/search bar out of view instead, trading one hidden element for another. This is not a regression: `'start'` alone couldn't have shown the button there either. The tabs and search bar are never scrolled out of view at any tested size, so the original below-the-fold bug does not come back — only the newer button-specific case can still require a small manual scroll, and only on unusually short viewports.
  - On a short panel (e.g. the Presets tab — no longer the default, but still reachable by switching tabs, closing, and reopening) the document may not have enough content below to scroll the panel flush to the viewport top — the browser scrolls as far as it can and stops there, which is expected, not a bug to chase.
...
**`preset-listbox`** (`preset-trigger` + `preset-listbox`) — the Common Compounds picker on the Presets tab, replacing a native `<select>` with `<optgroup>`s (removed entirely — see the `preset-select` note under Don't).

- **Why it exists:** a native `<select>` hands the picker off to the OS on mobile — a wheel or a full-screen overlay depending on platform — which can't be restyled and doesn't respect the page's width. Twenty presets across three groups (Common / Acids & Bases / Salts & Oxides) is exactly the shape of control that needs to stay in-page.
- **Pattern:** ARIA APG "Collapsible Dropdown Listbox" (select-only combobox). `preset-trigger` is a `<button role="combobox" aria-haspopup="listbox" aria-controls="preset-listbox" aria-expanded aria-activedescendant>`; `preset-listbox` is `role="listbox"`, with each compound `role="option"` and each section a `role="group" aria-label="<section name>"` wrapping its options (`group` is a valid owned element of `listbox` in ARIA 1.2 — this is the direct analogue of `<optgroup>`). DOM focus **never moves into the listbox** — it stays on `preset-trigger` the entire time the popup is open, and `aria-activedescendant` on the trigger tracks whichever option is currently highlighted. This is a deliberate deviation from the APG reference markup, which uses a focusable `<span>`/`<div role="combobox">` rather than a real `<button>` (native `<button>` isn't in the HTML-ARIA allowed-role list for an override to `combobox`) — a `<button>` was used anyway because it's what the task called for and it's what production combobox implementations (Radix, Reach UI, MUI Autocomplete) do in practice; verified against NVDA/VoiceOver-class support, not just against the letter of the ARIA-in-HTML table. Revisit if a stricter validator ever flags it.
- **Data:** reads `PRESETS`/`PRESET_GROUP_ORDER` from `src/utils/browseElements.ts` — the same `Preset[]` the "Build custom" formula bar's types come from — and builds both the group headings and the option rows from it at script load. There is no second, hardcoded copy of the compound list; `Preset.group` (added alongside this component) is what used to be each `<optgroup>`'s `label`.
- **Styling — trigger:** same recipe as `text-input`/`element-search`: `bg-surface-soft`, `inset-ring-1 inset-ring-hairline` at rest, `focus:inset-ring-2 focus:inset-ring-primary`, `rounded-[8px]`, `text-sm font-sans`, padding `pl-3 pr-8 py-[12px] mobile:py-[10px]` (right padding reserves the chevron, identical numbers to the old `preset-select`). This is a change from the retired `preset-select`, which carried a real `border` instead of `inset-ring` — see the Don't entry for why that carve-out no longer applies now that there's no native `<select>` chrome to fight. Placeholder text ("Select a compound…") renders `{colors.ink-tertiary}`, matching input placeholder color elsewhere.
- **Styling — popup:** `bg-surface-1` (one step up from the `panel-browse-elements` background it floats over, same ladder logic as `element-tile` sitting on the panel), `inset-ring-1 inset-ring-hairline`, `rounded-[8px]`, `max-h-72 overflow-y-auto`. **No drop shadow** — the surface-ladder-only elevation rule (see Elevation & Depth) applies here too; separation from the backdrop comes from the ring + the surface step, not a shadow.
- **Positioning — `position: fixed`, computed in JS, not `absolute` in CSS.** `#browse-panel` (several levels up the tree) carries `overflow-hidden` for its own rounded corners. A first pass made the popup `absolute` inside the `.relative` wrapper around the trigger — its containing block sat *inside* that clipping box, so the popup rendered but was cut off at `#browse-panel`'s edge (measured: only the first option partially visible, everything past it clipped away, on both a real mobile device and desktop — **not** a breakpoint-specific bug, since it was the same ancestor on every viewport). `position: fixed` was used instead: its containing block is the viewport itself, not the nearest positioned ancestor, so it escapes `overflow-hidden` on any ancestor that doesn't itself set `transform`/`filter`/`perspective`/`contain` (none in this tree do). `top`/`left`/`width` are read from `preset-trigger.getBoundingClientRect()` (already viewport-relative — the same coordinate space `fixed` uses, so no `scrollX`/`scrollY` offset needed, unlike positioning an `absolute` popup against document coordinates) and written as inline styles on open, and **re-read on every `scroll`/`resize` event while open** (capture-phase listeners on `window`, added on open and removed on close) rather than left as a one-time snapshot — a `fixed` element doesn't track the trigger moving under it on its own. Reposition, not close: an earlier version closed the popup on scroll instead, which raced with the Browse Elements panel's own open-time smooth-scroll (`browsePanel.scrollIntoView`, see `panel-browse-elements` above) — opening the listbox by keyboard while that unrelated animation was still mid-flight let its very next scroll tick close the popup the instant it opened. Repositioning is a no-op when the trigger hasn't actually moved (including when the *listbox's own* internal `overflow-y-auto` scrolls — scroll events don't bubble, but this capture-phase listener still sees them, and re-reading the trigger's unchanged rect is harmless), and is generally better UX than a dropdown that vanishes the moment the page scrolls at all. `toBeVisible()`/`boundingBox()` assertions on the listbox's own container did not catch the original clipping bug — both report the element's own layout box regardless of a clipping ancestor. Catching this class of bug in the test suite requires either comparing the popup's rendered height against its intended `max-h` (a severely clipped popup measures far short of it) or hit-testing content far from the trigger via `document.elementFromPoint` (clipped content isn't paintable/hit-testable at its own coordinates, even though it still has a computed bounding box) — see the Presets test file for both.
- **Group headings:** `text-xs font-semibold uppercase tracking-[0.4px] text-ink-subtle`, same type treatment as the "Common Compounds" label above the trigger — `aria-hidden`, since the heading is decorative; the group's accessible name comes from the `role="group"`'s own `aria-label`.
- **Option states:** highlighted (keyboard-active or hovered) gets `bg-surface-2`, same fill as `pill-inactive-hover`. A committed selection (`aria-selected="true"`) gets `bg-primary-soft` / `text-primary-pressed` instead — the exact reserved future use for `{colors.primary-soft}` noted under Colors — and this state is now genuinely reachable: reopening the picker after a selection shows the last-picked compound both `aria-selected` in the list and pre-highlighted (see the selection-behavior note below), where an earlier version reset the selection on every pick and so never left this state visible.
- **Selection behavior — no confirm button on this tab.** Clicking an option, or highlighting it and pressing Enter/Space, immediately writes `preset.molarMass` into the Molar Mass input (dispatching a real `input` event so the calculation re-runs) and then collapses the whole Browse Elements panel — the same end effect the old select-then-"Use this molar mass" flow had, just triggered by the selection itself rather than a second click. **Unlike that old flow, the selection is not reset afterward.** `preset-trigger-label`'s text changes from the placeholder to `"<name> (<formula>)"` (deliberately shorter than the option row's own text, which also carries `— <mass> g/mol` — redundant once Molar Mass itself shows the value) and stays that way — reopening the panel/listbox later shows the same label, and `openPresetListbox` pre-highlights that same option (`presetSelectedIndex`, not `0`) rather than starting from the top. Because the trigger's accessible name comes from `aria-labelledby="preset-label preset-trigger-label"`, updating this element's `textContent` keeps the accessible name in sync automatically — there's no separate `aria-label` to fall out of sync with it. The "Use this molar mass" button still exists on the sibling Build-custom tab (`use-custom`) for that tab's multi-element flow; it is unrelated to this control and was not touched.
- **Keyboard:** Arrow Down/Up open the popup (highlighting the first/last option if nothing is highlighted yet) or move the highlight by one when it's already open; Home/End jump to the first/last option; Enter/Space opens the popup if closed, or commits the highlighted option if open; Escape closes the popup without changing the selection, without collapsing the panel around it; Tab closes the popup and lets focus leave normally. Typeahead is implemented: any single printable keypress buffers into a 500ms-debounced string and jumps the highlight to the next option (wrapping) whose name starts with it, opening the popup first if needed.
- **Dismissal:** a capture-phase document click listener closes the popup on any click outside `preset-trigger`/`preset-listbox`, added only while open and removed on close (not left attached sitewide). Scroll and resize do **not** dismiss it — see the repositioning note above.
...
- **Formula bar** (Build custom tab): left/right split (Formula left, Molar Mass + "Use this molar mass" right-aligned) at `mobile:` (≥520px) and up, stacked and left-aligned below it, with the button going full-width. The container's own `flex-col mobile:flex-row` switch was fixed to the project's `mobile:` breakpoint early on, but two children's alignment classes were left unconditional (`text-right` on the Molar Mass block, `self-end` on the button) and kept producing a right-pushed look below 520px even after the container itself was already stacked — fixed by scoping both to `mobile:` (`mobile:text-right`) or swapping the axis-inappropriate value for one that means the same thing in a column layout (`self-end` → `self-stretch`, both overridden by `mobile:self-auto` at 520px and up regardless). Whenever a container's flex-direction is breakpoint-scoped, audit its children's own `self-*`/`text-*` classes for the same kind of leftover — the container switching doesn't automatically fix values on the children that were tuned for the old direction.
