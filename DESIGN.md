...
- **Don't add a real `border` back to any element documented here as `box-shadow: inset`.** This shipped as a real defect twice while `text-input`'s edge was still a `border`: once when focus alone changed width (0.5px → 2px) with nothing offsetting it, and once when a later 1px/2px resting/focus split reopened the same shift with nothing offsetting it either — both times because a real border's width adds directly to the box. Reintroducing `border` on any of these elements (even at a fixed width, even "just for one state") brings that box-model coupling back with it. If an element genuinely needs a real border again — e.g. because a forced-colors concern turns out to need more than the sitewide `[class*="inset-ring-"]` fallback in `global.css` — treat that as a fresh decision requiring its own padding math, not a quick swap back. **`preset-select` is the one case where this fresh decision was made** (native `<select>` chrome suppressed the box-shadow on real mobile engines) — it now carries `border border-hairline` as its *only* edge, no `inset-ring`. Don't re-add `inset-ring-*` alongside it (the two rendering together is exactly the doubled-edge defect this file's border-conversion section documents fixing), and don't remove the `border` in favor of `inset-ring` alone without re-verifying on real mobile `<select>` chrome first — `appearance: none` is believed to make the box-shadow reliable again, but that belief hasn't been confirmed on a real device, only in headless/emulated Chromium.
